...
#### No. 2.3 Rice cross-map DNA time series (with lagged CCM)
####
...
#for (edna_i in key_taxa_set){
  ...
  uic_res_tmp <- rUIC::uic(cbind(x_std, y_std, z1_std, z2_std, z3_std), lib_var = "x",
                           ...
                           E = Eopt + 1, tp = TP_RANGE, lib = rice_lib,
                           n_boot = 2000, seed = 1234) %>%
    cbind(data.frame(effect_var = rep("gr", length(TP_RANGE)),
                     cause_var =  rep(colnames(edna_all)[edna_i], length(TP_RANGE))), .,
  ...
  # Combine results
  uic_rice_edna0 <- rbind(uic_rice_edna0, uic_res_tmp)
...
# Rename object
uic_rice_edna <- uic_rice_edna0
rm(uic_rice_edna0)

# Save workspace and objects
saveRDS(uic_rice_edna, sprintf("%s/uic_rice_edna.obj", output_folder02))
save(list = ls(all.names = TRUE),
     file = sprintf("%s/%s_3.RData", output_folder02, output_folder02))

# Save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/%s_3_SessionInfo_%s.txt", output_folder02, substr(Sys.time(), 1, 10)))
