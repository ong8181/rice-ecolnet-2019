...
#### No. 2 Climate cross-map Rice time series (with lagged CCM)
####
...
# Prepare potential causal variable
y_std <- data.frame(y = as.numeric(scale(rice_all$gr)))
# Prepare output object
uic_clim_rice0 <- data.frame()
...
for (clim_i in clim_var_coln[1:5]){
  ...
  x_std <- data.frame(x = as.numeric(scale(clim_all[clim_i])))
  simp_res <- rUIC::simplex(cbind(x_std, y_std), lib_var = "x", cond_var = "y",
                            lib = clim_lib, E = E_RANGE, tp = 1, tau = 1, 
  ...
  for (tp_i in TP_RANGE){
    uic_res_tmp <- rUIC::uic(cbind(x_std, y_std), lib_var = "x", tar_var = "y",
                             E = Eopt + 1, tp = tp_i, lib = clim_lib,
                             n_boot = 2000, seed = 1234) %>%
      cbind(data.frame(effect_var = colnames(clim_all[clim_i])), cause_var = "gr", .)
    uic_clim_rice0 <- rbind(uic_clim_rice0, uic_res_tmp)
  }
...
# Visualize results
uic_clim_rice0$signif <- NaN
uic_clim_rice0$signif[uic_clim_rice0$pval <= 0.05] <- "P < 0.05"
uic_clim_rice0$signif[uic_clim_rice0$pval > 0.05] <- "N.S."
uic_clim_rice0$signif <- factor(uic_clim_rice0$signif, levels = c("P < 0.05", "N.S."))
selected_clim_var <- c("temp_mean", "relhm_mean", "light_mean", "actvp_mean", "satdf_mean")
uic_clim_rice <- uic_clim_rice0[!is.na(match(uic_clim_rice0$effect_var, selected_clim_var)),]
uic_clim_rice$effect_var <- factor(uic_clim_rice$effect_var,
                                  levels = c("temp_mean", "light_mean", "actvp_mean", "relhm_mean", "satdf_mean"))

g1 <- uic_clim_rice %>%
  ggplot(aes(x = tp, y = te, facet = effect_var)) + geom_line() +
  geom_point(aes(x = tp, y = te, color = signif)) +
  scale_color_manual(values = c("red3", "black"), name = NULL) +
  geom_hline(yintercept = 0, linetype = 2) + facet_wrap(~ effect_var) +
  ylab("TE") + xlab("UIC time lag (tp)") + ylim(-0.015, 0.065)

# Save workspace and ojcects
ggsave(sprintf("%s/uic_clim_rice.pdf", output_folder02), plot = g1, width = 10, height = 6)
saveRDS(uic_clim_rice, sprintf("%s/uic_clim_rice.obj", output_folder02))

save(list = ls(all.names = TRUE),
     file = sprintf("%s/%s_2.RData", output_folder02, output_folder02))

# Save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/%s_2_SessionInfo_%s.txt", output_folder02, substr(Sys.time(), 1, 10)))
