...
#### No.5 Check standard curve
####

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder05 <- "05_STDCheck_EukOut"
...
load("01_CERrice2019_EukSeqOut/01_CERrice2019_EukSeqOut.RData")
tax_claident <- read.csv("04_TaxaSTDcombine_EukOut/claident_tax_revise.csv")
sample_sheet <- read.csv("../sampledata_all/20200313_CMR-009_SampleSheet_EUK.csv")
...
std_copy_n <- c(20000, 10000, 5000, 2500, 500)
...
nrow(sample_sheet) == nrow(seqtab_nochim) # Should usually be TRUE; CMR-009, FALSE
...
g4 <- ggplot(seqtab_plot, aes(x = sample, y = value2)) +
  ...
  geom_segment(data = std_table_plot, aes(x = sample, xend = sample, y = min, yend = max),
               colour = "red3", size = 1.5, alpha = 0.5) + ggtitle("CMR-009, CERrice2019 Eukaryote seqs")
