...
#### No.5 Check standard curve (Prokaryote)
####

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder05 <- "05_STDCheck_ProkOut"
...
load("01_CERrice2019_ProkSeqOut/01_CERrice2019_ProkSeqOut.RData")
tax_claident <- read.csv("04_TaxaSTDcombine_ProkOut/claident_tax_revise.csv")
sample_sheet <- read.csv("../sampledata_all/20200315_CMR-010_SampleSheet_PRO.csv")
...
all(sample_sheet$Sample_Name2 == rownames(seqtab_nochim)) # Should be TRUE; RMR-076, FALSE
nrow(sample_sheet) == nrow(seqtab_nochim) # Should be TRUE; CMR-010, FALSE
...
g4 <- ggplot(seqtab_plot, aes(x = sample, y = value2)) +
  ...
  geom_segment(data = std_table_plot, aes(x = sample, xend = sample, y = min, yend = max),
               colour = "red3", size = 1.5, alpha = 0.5) + ggtitle("CMR-010, CERrice2019 Prokaryote seqs")
