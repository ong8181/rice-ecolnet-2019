...
#### No.4 STD sequences assignment (Fungi)
####

# Create output directory
dir.create("04_TaxaSTDcombine_FungiOut")

# Load Claident taxa assignment
claident_tax <- read.delim("03_ClaidentAssignTax_FungiOut/FungiASV_merge_classigntax")
head(claident_tax)
...
write.csv(claident_tax, "04_TaxaSTDcombine_FungiOut/claident_tax_revise.csv", row.names = F)

#### save session info
writeLines(capture.output(sessionInfo()),
           sprintf("00_SessionInfo/04_SessionInfo_FungiSTDident_%s.txt", substr(Sys.time(), 1, 10)))
