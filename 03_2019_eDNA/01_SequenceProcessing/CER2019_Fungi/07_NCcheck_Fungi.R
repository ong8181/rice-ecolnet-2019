...
#### No.7 CMR-011 Fungi: Negative & Positive control check
####

# Load workspace and data
load("06_ReadNSummary_FungiOut/06_ReadNSummary_FungiOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder07 <- "07_NCcheck_FungiOut"
...
p8 <- ggplot(ps_m7, aes(x = Sample, y = Abundance, fill = class)) +
  geom_bar(stat = "identity", colour = NA) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) + scale_fill_igv() +
