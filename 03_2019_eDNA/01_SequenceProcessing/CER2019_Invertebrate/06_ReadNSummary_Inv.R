...
#### No.6 CMR-011 Invertebrate: Read number visualization
####

# Load workspace and data
load("05_STDCheck_InvOut/05_STDCheck_InvOut.RData")

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder06 <- "06_ReadNSummary_InvOut"
...
write.csv(sample_summary, sprintf("%s/summary_sheet_Inv.csv", output_folder06))

# Repalce sample_sheet
sample_sheet <- sample_summary

# Save important objects
saveRDS(sample_sheet, sprintf("%s/sample_sheet_Inv.obj", output_folder06))
saveRDS(seqtab_nochim, sprintf("%s/seqtab_nochim_Inv.obj", output_folder06))
saveRDS(seqtab_conv, sprintf("%s/seqtab_conv_Inv.obj", output_folder06))
saveRDS(new_std_table, sprintf("%s/new_std_table_Inv.obj", output_folder06))
saveRDS(tax_claident, sprintf("%s/tax_claident_Inv.obj", output_folder06))
saveRDS(tax_claident2, sprintf("%s/tax_claident2_Inv.obj", output_folder06))
saveRDS(taxa_w_std, sprintf("%s/taxa_w_std_Inv.obj", output_folder06))
saveRDS(taxa_wo_std, sprintf("%s/taxa_wo_std_Inv.obj", output_folder06))
