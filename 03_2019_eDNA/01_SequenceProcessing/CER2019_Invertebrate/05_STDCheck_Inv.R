...
#### No.5 Check standard curve (Invertebrate)
####

# Set random seeds (for reproduction)
ran_seed <- 8181
set.seed(ran_seed)
output_folder05 <- "05_STDCheck_InvOut"
...
load("01_CERrice2019_InvSeqOut/01_CERrice2019_InvSeqOut.RData")
tax_claident <- read.csv("04_TaxaSTDcombine_InvOut/claident_tax_revise.csv")
sample_sheet <- read.csv("../sampledata_all/20200323_CMR-011_SampleSheet_INV.csv")
...
std_copy_n <- c(400, 200, 100, 50, 10)
...
all(sample_sheet$Sample_Name2 == rownames(seqtab_nochim)) # Should be TRUE; RMR-099, FALSE
nrow(sample_sheet) == nrow(seqtab_nochim) # Should be TRUE; CMR-011_Inv, FALSE
...
g4 <- ggplot(seqtab_plot, aes(x = sample, y = value2)) +
  ...
  geom_segment(data = std_table_plot, aes(x = sample, xend = sample, y = min, yend = max),
               colour = "red3", size = 1.5, alpha = 0.5) + ggtitle("CMR-011, CERrice2019 Invertegrate seqs")
